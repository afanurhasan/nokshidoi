        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 mt-12">
          {steps.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={`relative flex flex-col justify-between p-5 sm:p-6 rounded-2xl border transition-all duration-300 shadow-xs hover:shadow-lg ${
                  isAdvance
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 mb-4">
                  <span className="px-2.5 py-1 rounded-full text-xs font-black bg-amber-800 text-white shadow-2xs">
                    {item.step}
                  </span>
                  <div className={`p-2 rounded-xl ${
                    isAdvance ? 'bg-amber-200 text-amber-900' : isFullPayment ? 'bg-blue-200 text-blue-900' : 'bg-amber-100 text-amber-800'
                  }`}>
                    <IconComp size={20} />
                  </div>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-200/60 text-[11px] font-bold text-slate-700">
                  ...
                  <p className="mt-0.5 text-slate-800">{item.note}</p>
                </div>
              </div>
            );
          })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-5 bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/15 text-center flex flex-col items-center justify-center space-y-4">
              <span className="text-xs font-black uppercase tracking-wider text-amber-300">
                অফিশিয়াল পেমেন্ট নম্বর
              </span>
              <div className="text-2xl sm:text-3xl font-black text-white tracking-wider bg-stone-950/70 px-6 py-2.5 rounded-xl border border-amber-400/50">
                {BUSINESS_CONFIG.paymentNumber}
              </div>
              <p className="text-xs text-amber-100/80 max-w-xs font-medium">
                (অগ্রিম ৩০% ও গাড়ি ছাড়ার পূর্বে বাকি ৭০% এবং প্রযোজ্য রোড ভাড়া এই নম্বরে পরিশোধযোগ্য)
              </p>
